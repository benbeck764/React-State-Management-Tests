...
export const geniusApi = createApi({
  ...
  endpoints: (builder) => ({
    geniusSearch: builder.query<GeniusApiResponse<GeniusSearchResponse>, string>({
      query: (query: string) => ({
    ...
    getGeniusSong: builder.query<GeniusApiResponse<GeniusSong>, GeniusSongRequest>({
    ...
    getGeniusLyrics: builder.query<string[], string>({
      queryFn: async (geniusUrl: string) => {
        try {
          ...
          const res = await axios.get(geniusUrl.replace('https://genius.com', '/lyrics'));
          if (res.status >= 200 && res.status <= 299) {
            ...
            const parser = new htmlparser2.Parser(
              {
                ontext: (text) => {
                  if (insideLyricsContainer && !inExclusionSection) {
                    if (firstSection) {
                      firstSection = false;
                    } else {
                      // Add some space after each song "section", except the first
                      if (text.endsWith(']')) lyrics.push(...['', '', '', '', '']);
                    }
                ...
                onopentag: (name, attribs) => {
                  if (name === 'div' && attribs['data-lyrics-container'] === 'true')
                    insideLyricsContainer = true;

                  if (name === 'div' && attribs['data-exclude-from-selection'] === 'true')
                    inExclusionSection = true;
                },
                onclosetag: (tagname) => {
                  if (tagname === 'div' && insideLyricsContainer) insideLyricsContainer = false;
                  if (tagname === 'div' && inExclusionSection) inExclusionSection = false;
                }
              },
              {
                decodeEntities: true
...
export const { useGeniusSearchQuery, useGetGeniusSongQuery, useGetGeniusLyricsQuery } = geniusApi;
