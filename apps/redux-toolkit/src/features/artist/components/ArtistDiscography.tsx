...
const ArtistDiscography: FC<ArtistDiscographyProps> = (props: ArtistDiscographyProps) => {
  ...
  const { data: multipleAlbums, isFetching: loadingMultipleAlbums } = useGetMultipleAlbumsQuery(
    {
      /* eslint-disable-next-line @typescript-eslint/no-non-null-asserted-optional-chain */
      ids: artistAllAlbumsResponse?.items?.map((a: SimplifiedSpotifyAlbum) => a.id)!
    },
    {
  ...
  const allCompilations = artistAllAlbumsResponse?.items?.filter(
    (a: SimplifiedSpotifyAlbum) => a.album_type === 'compilation' && a.album_group === 'compilation'
  ) as SpotifyAlbum[] | undefined;
  ...
  const discographyTypes: DiscographyType[] = ['all', 'album', 'single', 'compilation'];
  ...
  const selected = discographyState.find((s: DiscographyState) => s.selected === true);

  const discographyTypeExists = (type: DiscographyType) => {
    let exists = false;
    switch (type) {
      case 'all':
        exists = typeof popularReleases !== 'undefined' && popularReleases.length > 0;
        break;
      case 'album':
      ...
      case 'single':
        ...
        break;
      case 'compilation':
        exists = typeof allCompilations !== 'undefined' && allCompilations.length > 0;
        break;
    }
      ...
      <Stack>
        <Stack direction="row" alignItems="center" justifyContent="space-between">
          ...
          <AppLink to={getDiscographyUrl(artist.id, selected?.type ?? 'all')}>
            <Typography variant="paragraphBold" sx={{ color: (theme) => theme.palette.grey[400] }}>
              Show all
            </Typography>
          </AppLink>
        </Stack>
                ...
                <Chip
                  ...
                  label={
                    <Typography
                      ...
                      sx={{
                        color: (theme) =>
                          s.selected ? theme.palette.grey[800] : theme.palette.text.primary
                      }}
                    >
                      {s.title}
                    </Typography>
                  ...
                  sx={{
                    ...
                    backgroundColor: (theme) =>
                      s.selected ? theme.palette.text.primary : theme.palette.grey[800],
                    '&:hover': {
                      backgroundColor: (theme) =>
                        s.selected ? theme.palette.text.primary : theme.palette.coolGrey[800]
                    }
                  }}
                />
              );
        ...
        <AlbumsGrid
          ...
          loading={loading}
          latestRelease={selected?.type === 'all' || selected?.type === latestRelease?.album_type}
