...
const TrackArtistsPopularItems: FC<TrackPopularAlbumsProps> = (props: TrackPopularAlbumsProps) => {
  const { artist, onAlbumSelected } = props;

  const { data: artistAlbumsResponse, isFetching: loadingArtistAlbums } = useGetArtistAlbumsQuery(
    // eslint-disable-next-line @typescript-eslint/no-non-null-asserted-optional-chain
    { id: artist?.id!, limit: 50, include_groups: 'album,single' }, // Max 50 per API spec
    { skip: !artist }
  );

  const { data: multipleAlbums, isFetching: loadingPopularAlbums } = useGetMultipleAlbumsQuery(
    {
      /* eslint-disable-next-line @typescript-eslint/no-non-null-asserted-optional-chain */
      ids: artistAlbumsResponse?.items?.map((a: SimplifiedSpotifyAlbum) => a.id)!
    },
    {
      ...
      <Stack gap={1}>
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          <Stack direction="row" alignItems="center" gap={1}>
            ...
            <Stack>
              <TypographySkeleton variant="paragraphExtraSmallBold" charCount={16} />
              <TypographySkeleton variant="h4" charCount={22} charCountVariance={8} />
            </Stack>
          </Stack>

          <TypographySkeleton variant="paragraphSmallBold" charCount={8} />
        </Stack>
        <AlbumsGrid data={undefined} loading={true} pageSize={6} cardType="minimal" />
      </Stack>
    );
  } else {
    return (
