...
const Track: FC = () => {
  ...
  const { data: artistsResponse, isFetching: loadingArtists } = useGetArtistsQuery(
  ...
  return (
    <Stack>
      <TrackHeader loading={loadingTrack || loadingArtists} track={track} artist={artist} />
      <Box my={2}>
      ...
      {/* API was deprecated, so not using it anymore */}
      {/* <Box my={1.5}>
      ...
      {otherArtists?.map((otherArtist: SpotifyArtist) => (
        <Box key={otherArtist.id} my={1.5}>
          <TrackArtistsPopularItems artist={otherArtist} onAlbumSelected={handleAlbumSelected} />
        </Box>
      ))}
      {/* API was deprecated, so not using it anymore */}
      {/* <Box my={1.5}>
