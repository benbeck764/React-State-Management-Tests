...
import { getAlbumUrl, getArtistUrl, getTrackUrl } from '../../routing/common/url';
...
import { SpotifyDevice, SpotifyTrack } from '../../state/queries/models/spotify.models';
...
const Player: FC = () => {
  ...
  const playingOnOtherDevice = typeof hasActiveDevice !== 'undefined' && !isActiveDevice;

  useEffect(() => {
    if (!hasActiveDevice && currentDeviceId)
      void transferPlayback({ deviceIds: [currentDeviceId], play: false });
  }, [hasActiveDevice, currentDeviceId, transferPlayback]);

  // [TODO]: Do this for now, implement Episodes later?
  const item =
    (playbackState?.item as SpotifyTrack) ?? (currentlyPlayingRes?.item as unknown as SpotifyTrack);
