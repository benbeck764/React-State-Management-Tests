...
const Search: FC = () => {
  ...
  const topTracks = searchResult ? [...searchResult.tracks.items].splice(0, 4) : [];
  const topArtists = searchResult ? [...searchResult.artists.items].splice(0, 6) : [];
  const topAlbums = searchResult ? [...searchResult.albums.items].splice(0, 6) : [];
    ...
    return (
      <Stack gap={6}>
        ...
        <Stack>
          ...
          <ArtistsGrid data={undefined} cardVariant="large" pageSize={6} loading={true} />
        </Stack>
        <Stack>
