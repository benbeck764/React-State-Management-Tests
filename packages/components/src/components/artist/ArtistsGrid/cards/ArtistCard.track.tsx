...
const ArtistCardTrack: FC<ArtistCardProps> = (props: ArtistCardProps) => {
      ...
      <Box sx={{ p: 1, borderRadius: '4px' }}>
        <Stack direction="row" alignItems="center" gap={2}>
          ...
          <Stack>
            ...
            <TypographySkeleton variant="paragraphBold" charCount={12} charCountVariance={6} />
          </Stack>
        </Stack>
      </Box>
    );
  ...
  const { artist, renderLink } = props;
  const LinkComponent = renderLink?.(artist) ?? Fragment;
  ...
  return (
    <Box
      sx={{
        p: 1,
        borderRadius: '4px',
        '&:hover': { backgroundColor: (theme) => theme.palette.coolGrey[800] }
      }}
    >
      <Stack direction="row" alignItems="center" gap={2}>
