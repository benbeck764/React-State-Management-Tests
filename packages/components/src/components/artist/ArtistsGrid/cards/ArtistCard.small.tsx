...
import Equalizer from '@/components/common/Equalizer/Equalizer';
...
const ArtistCardSmall: FC<ArtistCardProps> = (props: ArtistCardProps) => {
  ...
  const { artist, playButtonProps } = props;
  const { playbackState, deviceId } = playButtonProps || {};

  const isCurrentArtist = playbackState && artist.uri === playbackState.context?.uri;
  ...
  return (
    ...
    >
      <Stack direction="row" position="relative" alignItems="center" justifyContent="space-between">
        <Stack direction="row" alignItems="center" gap={1}>
          <Avatar variant="rounded" sx={{ width: 48, height: 48 }} src={artist.images[0].url} />
          <StyledEllipsingTextContainer
            lines={1}
            reserveHeight={
